"""
Trustpilot enrichment via n8n csp-trustpilot-enrichment workflow.

Usage:
    python scripts/enrich_trustpilot.py [--vendor WEBSITE] [--limit N] [--all]

Posts vendor batch to the n8n Trustpilot enrichment webhook. The workflow
scrapes Trustpilot review pages and writes trustpilot_rating and
trustpilot_review_count back to cs_vendors via the admin API.
"""
...
WEBHOOK_PATH = "csp-trustpilot-enrichment"
...
def main(vendor_website: str = "", limit: int = 0, skip_existing: bool = True) -> int:
    ...
    vendors = _sb_get(
        "cs_vendors?select=website,name,trustpilot_rating"
    ...
    if skip_existing:
        vendors = [v for v in vendors if v.get("trustpilot_rating") is None]
    if limit:
